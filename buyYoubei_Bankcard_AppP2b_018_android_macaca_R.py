...
class AppP2b(unittest.TestCase):
    ...
    def buy_zhuanbei(self):
        ...
        sleep(2)
        driver.touch('drag',{'fromX':520,'fromY':1700,'toX':520,'toY':1200})
        sleep(2)
        ...
        driver.element_by_id("com.richfinancial.pujiaosuo:id/invest_pay_value").send_keys("4500")
        sleep(1)
        #e-Bai
        ...
        sleep(3)
        now=time.strftime("%Y-%m-%d %H_%M_%S")
        sf4="./"+now+"_018_recharge_records_macaca_R.png"
        ...
        sleep(4)
        now=time.strftime("%Y-%m-%d %H_%M_%S")
        sf5="./"+now+"_018_recharge_detail_macaca_R.png"
...
if __name__ == '__main__':
    ...
    now=time.strftime("%Y-%m-%d %H_%M_%S")
    filename="./"+now+"_018_result_macaca_R.html"
    fp=open(filename,"wb")
    runner=HTMLTestRunner(stream=fp,title='51p2b of App environment android6.0.1[投资优贝(用银行卡支付:融宝支付)] test case report by Macaca',
