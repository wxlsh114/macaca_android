...
class AppP2b(unittest.TestCase):
    ...
    def buy_zhuanbei(self):
        ...
        sleep(2)
        driver.element_by_name("净化器赚呗 2期").click()
        sleep(2)
        driver.element_by_name("立即投资").click()
        sleep(3)
        driver.element_by_id("com.richfinancial.pujiaosuo:id/invest_pay_value").click()
        driver.element_by_id("com.richfinancial.pujiaosuo:id/invest_pay_value").send_keys("1600")
        sleep(2)
        #e-Bai
...
if __name__ == '__main__':
    ...
    now=time.strftime("%Y-%m-%d %H_%M_%S")
    filename="./"+now+"_025_result_macaca_R.html"
    fp=open(filename,"wb")
    runner=HTMLTestRunner(stream=fp,title='51p2b of App environment android6.0.1(投资净化器赚呗(用账户余额支付) test case report by Macaca',
